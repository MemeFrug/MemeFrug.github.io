...
const ENGINE = {
    ...
    */
    Draw: () => {
        ...
        //Draw Everything inside the chunks
        for (let s = 0; s < chunksToShow.length; s++) {
            const element = chunksToShow[s];
            const i = element.i
            const j = element.j
...
class Square extends gameObject {
    constructor(isStatic = false, x = 0, y = 0, w = 50, h = 50, mass = 0) {
        ...
        if (!this.isStatic) { // Check if the cube should be updated
            ...
            //Put in fucking uhhh chunksDrawLoop
            let x2 = 0
            let y2 = 0
            for (let i = 0; i < ENGINE.chunksDrawLoop.length; i++) {
                x2 = 0
