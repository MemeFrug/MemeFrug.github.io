...
const WORLD = {
     ...
     */
    ChangeWorldSize(w, h, dataToAddOnto = WORLD.data) {
        return new Promise((resolve) => {
            ...
            this.data = dataToAddOnto
            console.log(WORLD.blockSize / this.data.length, H);
            for (let j = this.data.length * WORLD.blockSize; j <= H; j++) {
                let iData = this.data[j]
                for (let i = this.data[j].length * WORLD.blockSize; i <= W; i++) {
                    iData.push(0)
                }
