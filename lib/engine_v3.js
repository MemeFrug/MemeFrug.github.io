...
const ENGINE = {
    ...
    Config: {
        ...
        sideScrollPlayerPositionOffset: {
            x: 0,
            y: 0
        },
        ...
        gravity: 2000, // (was 1600) The Worlds Gravity
    },
    Load: {
        loadElements: [],
        _: () => {
    ...
    */
    ...
    generateChunks: () => {
        return new Promise((resolve, reject) => {
            //Generate Each Chunk
            const amtOfChunksH = Math.ceil(WORLD.data.length / Math.ceil(ENGINE.Config.chunkSize / WORLD.blockSize))
            const amtOfChunksW = Math.ceil(WORLD.data[0].length / Math.ceil(ENGINE.Config.chunkSize / WORLD.blockSize))
    
            for (let i = 0; i < amtOfChunksH; i++) {
                let chunkRow = []
                for (let j = 0; j < amtOfChunksW; j++) {
                    chunkRow.push([])
                }
                ENGINE.chunksDrawLoop.push(chunkRow)
            }
            resolve()
        });
    },
    ...
    */
    ...
    detectCollisions: (obj1, obj22) => {
        if (obj22) {
            obj1.collisionDetection(obj22)
        } else {
            return;
        }
    ...
    */
    Draw: (deltaTime) => {
        const ctx = ENGINE.canvas.ctx // Get the context to draw on
        ...
        if (ENGINE.Config.sideScroller) {
            ...
            if (!LocalPlayer.error) {
                ENGINE.VARIABLES.Cam.x += (-LocalPlayer.x * ENGINE.Config.cameraScale - LocalPlayer.w / 2 + ENGINE.Config.sideScrollPlayerPositionOffset.x + nativeWidth /  ENGINE.Config.sideScrollPlayerPosition - ENGINE.VARIABLES.Cam.x) * ENGINE.Config.cameraFollowSpeed * ENGINE.VARIABLES.oldDeltaTime
                ENGINE.VARIABLES.Cam.y += (-LocalPlayer.y * ENGINE.Config.cameraScale - LocalPlayer.h / 2 + ENGINE.Config.sideScrollPlayerPositionOffset.x + nativeHeight / ENGINE.Config.sideScrollPlayerPosition - ENGINE.VARIABLES.Cam.y) * ENGINE.Config.cameraFollowSpeed * ENGINE.VARIABLES.oldDeltaTime
                ENGINE.VARIABLES.Cam.x = clamp(ENGINE.VARIABLES.Cam.x, -WORLD.size.w + nativeWidth - ENGINE.Config.sideScrollerSideOffset.right, ENGINE.Config.sideScrollerSideOffset.left)
                ENGINE.VARIABLES.Cam.y = clamp(ENGINE.VARIABLES.Cam.y, -WORLD.size.h + nativeHeight - ENGINE.Config.sideScrollerSideOffset.bottom, ENGINE.Config.sideScrollerSideOffset.top)
        ...
        ctx.scale(ENGINE.Config.cameraScale, ENGINE.Config.cameraScale); // Set Scale to 1:1
        ...
        let x = 0
        let y = 0
        for (let i = 0; i < ENGINE.chunksDrawLoop.length; i++) {
            x = 0
            for (let j = 0; j < ENGINE.chunksDrawLoop[i].length; j++) {
                // Check the chunks in view
                if (rectIntersect(x, y, ENGINE.Config.chunkSize, ENGINE.Config.chunkSize, -ENGINE.VARIABLES.Cam.x, -ENGINE.VARIABLES.Cam.y, ENGINE.Config.nativeWidth, ENGINE.Config.nativeHeight)) {
                    for (let k = 0; k < ENGINE.chunksDrawLoop[i][j].length; k++) {
                        const object = ENGINE.chunksDrawLoop[i][j][k];
                        ENGINE.detectCollisions(object, ENGINE.gameObjects[0]) // Do collisions
                        for (let k = k+1; k < ENGINE.chunksDrawLoop[i][j].length; k++) {
                        }
                        if (object.Draw != undefined) object.Draw(ctx) // If the Object has a draw function, run it
                    }

                    strokeRect(x, y, ENGINE.Config.chunkSize, ENGINE.Config.chunkSize, "red")
                }
                x += ENGINE.Config.chunkSize
            }
            y += ENGINE.Config.chunkSize
        }
    ...
    */
    Update: (timeStamp) => {
        ...
        ENGINE.Draw(deltaTime) // Draw Function, Used For Drawing All Elements Onto the canvas
 ...
 */
function createCanvas(autoResize = true, resizeType = Enum.ResizeType.AspectRatio, width = 500, height = 500) {
...
class gameObject {
    constructor(isStatic = false, x = 0, y = 0, mass = 0) {
        ...
        this.chunksDrawLoopChunk = {i: 0, j: 0}
    ...
    Destroy() {
        ...
        // const chunkIndex = ENGINE.chunksDrawLoop[this.chunksDrawLoopChunk.i][this.chunksDrawLoopChunk.j].indexOf(this);
        ...
        // if (ENGINE.chunksDrawLoop[this.chunksDrawLoopChunk.i][this.chunksDrawLoopChunk.j][chunkIndex]){ // Checks if this variable exists
        //     ENGINE.chunksDrawLoop[this.chunksDrawLoopChunk.i][this.chunksDrawLoopChunk.j].splice(chunkIndex, 1)
        //     console.log(ENGINE.chunksDrawLoop[this.chunksDrawLoopChunk.i][this.chunksDrawLoopChunk.j][chunkIndex]);
        // }
...
class Square extends gameObject {
    constructor(isStatic = false, x = 0, y = 0, w = 50, h = 50, mass = 0) {
        ...
        this.isColliding = false;
        this.customCollision = undefined
        ...
        if (!this.isStatic) { // Check if the cube should be updated
            ...
            for (let i = 0; i < ENGINE.chunksDrawLoop.length; i++) {
                x2 = 0
                for (let j = 0; j < ENGINE.chunksDrawLoop[i].length; j++) {
                    if (rectIntersect(x2, y2, ENGINE.Config.chunkSize, ENGINE.Config.chunkSize, x, y, w, h)) {
                        ENGINE.chunksDrawLoop[i][j].push(this)
                        this.chunksDrawLoopChunk = {i: i, j: j}
                    }
                    x2 += ENGINE.Config.chunkSize
                }
                y2 += ENGINE.Config.chunkSize
            }
        }
    ...
    collisionDetection(obj) {
        if (obj.customCollision) return;
        
        if (this.customCollision) {
            this.customCollision(obj)
            return
        }
    ...
    Update(deltaTime) {
        ...
        if (!this.isStatic) {
            ...
            if (this.vy < this.gravityMax) this.vy += ENGINE.Config.gravity * deltaTime;
...
const WORLD = {
    ...
    blockSize: 50, // The size of the blocks
    ...
    /**
     * Change the size of the world in cubes
     * @param {*} w Width Of The World (Will be divided by the tile size)
     * @param {*} h Height Of The World (Will be divided by the tile size)
     */
    ChangeWorldSize(w, h, data = WORLD.data) {
        return new Promise((resolve) => {
            if (w < data[0].length) {
                for (let i = 0; i < data.length; i++) {
                    const element = data[i];
                    data[i].splice(w, element.length - w)
                }
            } else if (w > data[0].length) {
                const amountToAdd = w - data[0].length
                for (let i = 0; i < data.length; i++) {
            ...
            if (h < data.length) {
                data.splice(h, data.length - h)
            } else if (h > data.length) {
                const amountToAdd = h - data.length 
