...
beforeUpdate = function (deltaTime) { return } // DISCONTINUED
afterUpdate = function (deltaTime) { return } // DISCONTINUED
update = function (deltaTime) {return}
draw = function (ctx) { return }
loaded = function () { return }
...
const ENGINE = {
    ...
    canvas: {
        ...
        addEvents: (resize = false) => {
            ...
            document.addEventListener("mousedown", (e) => {
                e.preventDefault()
                if (e.button == 0)
                    ENGINE.VARIABLES.LeftClick = true
                else if (e.button == 2)
                    ENGINE.VARIABLES.RightClick = true
            }); // Get Mouse Position
            document.addEventListener("mouseup", (e) => {
                e.preventDefault()
                if (e.button == 0)
                    ENGINE.VARIABLES.LeftClick = false
                else if (e.button == 2)
                    ENGINE.VARIABLES.RightClick = false
            }); // Get Mouse Position
    ...
    */
    ...
    getMouseLeftClick: () => {
        return ENGINE.VARIABLES.LeftClick;
    },

    getMouseRightClick: () => {
        return ENGINE.VARIABLES.RightClick;
    },
    ...
    */
    Update: (timeStamp) => {
        ...
        beforeUpdate(deltaTime) // Calling Custom Update Function // DISCONTINUED
        update(deltaTime)
        ...
        afterUpdate(deltaTime) // Calling Custom After Update Function // DISCONTINUED
...
function fillText(context, text, x, y, maxWidth = undefined, lineHeight = undefined) {
    if (!maxWidth) {
    ...
    init(data = WORLD.data) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            for (let j = 0; j < data[i].length; j++) {
                if (data[i][j] == 1) {
                    let dirt = new Square(true, x, y, 50, 50);
...
/**
 * TODO: CONVERT THIS TO AN OBJECT NOT A CLASS
